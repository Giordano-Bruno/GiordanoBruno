...
require_once("../shared/global_constants.php");//jalg multi 5-7-2015
...
class Settings {
  ...
  ****************************************************************************
  */
static function getLocales () {//jalg multi 5-7-2015
//  function getLocales () {
    ...
    while (false!==($file=readdir($dir_handle))) {
      if ($file != '.' && $file != '..') {
		...
		if (isset($lang_metadata['locale_description'])){//jalg multi 5-7-2015
  ...
  static function getLocaleMeta($locale){ //jalg multi 5-7-2015
    if (file_exists(OBIB_LOCALE_ROOT.'/'.$locale.'/metadata.php')) {
        include(OBIB_LOCALE_ROOT.'/'.$locale.'/metadata.php');
		if (isset($lang_metadata['locale_description'])){
			$arr_temp = array($locale => $lang_metadata['locale_description']);
			return $lang_metadata + $arr_temp;
		}
    }
	return NULL;
  }
  ...
  function getLibraryAders() {
    return $this->_libraryAders;
  }
  ...
  function setLibraryAders($value) {
    $this->_libraryAders = trim($value);
  }
