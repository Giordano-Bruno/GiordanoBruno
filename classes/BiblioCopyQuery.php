...
require_once("../classes/BiblioFieldQuery.php");//revisar si es necesaria  // jalg multi 5-7-2015 ALX
...
class BiblioCopyQuery extends Query {
  var $_rowCount = 0;
//  var $_loc;
  static $_loc = NULL; // jalg multi 5-7-2015 ALX

  function __construct() { // jalg multi 5-7-2015 ALX
    parent::__construct();
...
//    $this->Query();
  ...
  function fetchCopy() { // jalg multi 5-7-2015 ALX
...
*/
  ...
  function _mkObj(&$array, &$customs=NULL) { // jalg multi 5-7-2015 ALX
    $copy = new BiblioCopy();
    $copy->setBibid($array["bibid"]);
    $copy->setCopyid($array["copyid"]);
    $copy->setCreateDt($array["create_dt"]);
...
/*
  function _mkObj($array) {//add jalg
    $copy = new BiblioCopy();
    $copy->setBibid($array["bibid"]);
    $copy->setCopyid($array["copyid"]);
    $copy->setCreateDt($array["create_dt"]);
...
*/
  ...
  function getCustomFields( $bibid, $copyid ) { // jalg multi 5-7-2015 ALX
    $sql = $this->mkSQL('select * from biblio_copy_fields '
      . 'where bibid=%N ', $bibid);
	if (!empty($copyid)){
		$sql .=$this->mkSQL(' and copyid=%N', $copyid);
	}
    $rows = $this->selectFull($sql);
    $fields = array();
	if (!empty($copyid)){
    while ($r = $rows->next()) {
      $fields[$r['code']] = $r['data'];
		...
		while ($r = $rows->next()) {
			if ( !array_key_exists($r['copyid'], $fields) )
				$fields[$r['copyid']] = array();
			$fields[$r['copyid']][$r['code']] = $r['data'];
		}
	}
    return $fields;
  }
...
*/
  ...
  function setCustomFields($bibid, $copyid, $fields) { // jalg multi 5-7-2015 ALX
...
//add -- jalg pendinte depurar
   ...
   ****************************************************************************
   */
  function insert($copy) { // jalg multi 5-7-2015 ALX
    ...
    $sql = $this->mkSQL("insert into biblio_copy values (%N"
                        . ",null, now(), %Q, %S, %Q, sysdate() "
						. ",%S , %S, 0)", //Default renewal count to zero
                        $copy->getBibid(), $copy->getCopyDesc(),
                        $copy->getBarcodeNmbr(), $copy->getStatusCd()
						, $copy->getDueBackDt(), $copy->getMbrid()
						);
    $ret = $this->act($sql); //, $this->_loc->getText("biblioCopyQueryErr3"));
    if (!$ret) {
      return $ret;
    }
    $copyid = $this->getInsertID();
	$copy->setCopyid($copyid);
    $this->setCustomFields($bibid, $copyid, $copy->_custom);
	if (!empty($copy->_usMARC_hold)){
		//echo '<h3> insert hold tags '.json_encode($copy->_usMARC_hold).'</h3>';
		BiblioFieldQuery::insertFields($bibid, $copyid, $copy->_usMARC_hold, $this);
	}
    return $ret;
  }

/*
  function insert($copy) { //add jalg
    # checking for duplicate barcode number
    $dupBarcode = $this->_dupBarcode($copy->getBarcodeNmbr());
    if ($this->errorOccurred()) return false;
    if ($dupBarcode) {
...
*/
  ...
  function update($copy) { // jalg multi 5-7-2015 ALX
...
*/
  ...
  function delete($bibid,$copyid=0) { // jalg multi 5-7-2015 ALX
...
*/
   ...
   ****************************************************************************
   */
  function _getCollectionInfo($bibid) { // jalg multi 5-7-2015 ALX
