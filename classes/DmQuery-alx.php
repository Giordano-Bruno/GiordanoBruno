...
class DmQuery extends Query {
  ...
  function _get($table, $code = "") {
    $this->_tableNm = $table;
    $sql = $this->mkSQL("select *, %L as description from %I ", $table.".description", $table);
    if ($code != "") {
  ...
  function insert($table, $dm) {
    $sql = $this->mkSQL("insert into %I ", $table);
    if ($table == "collection_dm"
        or $table == "material_type_dm"
        or $table == "mbr_classify_dm") {
      $sql .= 'set code=null, ';
    } else {
      $sql .= $this->mkSQL('set code=%Q, ', $dm->getCode());
    }
    
    $sql .= $this->mkSQL("description=%Q", $dm->getDescription());
    if (strlen(VIEW_LOCALE) > 0){
        $sql .= $this->mkSQL(", description_%i = %Q", VIEW_LOCALE, $dm->getDescription());
    }
    $sql .= ", default_flg='N' ";
    if ($table == "collection_dm") {
  ...
  function update($table, $dm) {
    $sql = $this->mkSQL("update %I set default_flg='N' ",
