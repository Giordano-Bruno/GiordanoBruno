...
." b.bibid,null,  null,     b.barcode_nmbr,ifnull(bs.status_cd,'in'),ifnull(bs.status_begin_dt,b.create_dt),bs.due_back_dt,bs.mbrid from "
  ...
  /* Upgrade 0.7.1 to 0.8.0 */
  function _upgrade080_e($prfx, $tmpPrfx) {
    //$settings = $this->exec('select * from '.$prfx.'settings ');
    //$locale = $settings[0]['locale'];
    $version = '0.8.0';
    $locale = $this->getCurrentLocale();
    if (!is_dir('../locale/'.$locale.'/sql/'.$version.'/domain')) {
      $locale = "en";
    }

    //$version = $this->getCurrentDatabaseVersion($toTablePrfx);
    $rootDir = '../install/' . $version;
    $localeDir = '../locale/' . $locale . '/sql/' . $version;
    
    $this->act('alter ignore table '.$prfx.'biblio_copy_fields_dm change code code varchar(16) character set ascii');
    $this->act('alter ignore table '.$prfx.'biblio_status_dm change code code varchar(3) character set ascii');
    $this->act('alter ignore table '.$prfx.'member_fields_dm change code code varchar(16) character set ascii');
    $this->act('alter ignore table '.$prfx.'transaction_type_dm change code code varchar(2) character set ascii');
    
	$this->executeSqlFile($rootDir.'/lang.sql', $prfx, true);
	$this->executeSqlFile($localeDir . '/domain/lang.sql', $prfx, true);

    $sql = $this->mkSQL('update ignore '.$prfx.'lang set default_flg=\'Y\' where id = %Q', $locale);
    $this->act($sql);

	/*$charset = $this->getCurrentCharset($prfx);
	$localizator = new LocaleQuery($locale, $charset, $prfx, $version);
    $localizator->PrepareLocaleSeance($locale, $charset);*/

    $this->exec("update settings set version='0.8.0'");
    $notices = array();
    return array($notices, NULL);
  }
  ...
  function _upgrade081_e($prfx, $tmpPrfx) {
    ...
    $this->act('alter ignore table '.$prfx.'biblio_copy change barcode_nmbr barcode_nmbr varchar(20) null');

    $this->exec("update settings set version='0.8.1'");
    $notices = array();
    return array($notices, NULL);
  }

  function _upgrade090_e($prfx, $tmpPrfx) {
    //$settings = $this->exec('select * from '.$prfx.'settings ');
    //$locale = $settings[0]['locale'];
    $version = '0.9.0';
    $locale = $this->getCurrentLocale();
    ...
    $this->act('alter ignore table '.$prfx.'biblio_field drop primary key, add primary key (fieldid)');
	if (is_null($this->select01("SHOW INDEX FROM biblio_copy WHERE KEY_NAME = 'copy_key'")) ){
		$this->act('alter ignore table '.$prfx.'biblio_copy  add unique index copy_key(copyid)');
	}
    $this->executeSqlFile($rootDir.'/sql/hold_field.sql', $prfx, true);

    $this->exec("update settings set version='0.9.0'");
    $notices = array();
    return array($notices, NULL);
  }
}
