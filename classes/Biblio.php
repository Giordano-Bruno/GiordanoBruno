...
//  var $_loc;
  ...
  var $_fieldsHoldingBarcode; // jalg multi 5-7-2015 ALX

  function Biblio () {
	if (is_null($this->_loc))// jalg multi 5-7-2015 ALX
    $this->_loc = new Localize(OBIB_LOCALE,"classes");
	$this->_fieldsHoldingBarcode = array(// jalg multi 5-7-2015 ALX
		  '852p'	//marc21
		, '949i'	//OCLC holding
		, '952p'	//koha holding// jalg multi 5-7-2015 ALX
    );
  }
  ...
  function haveHoldingInfo(){
	//check that have any tag mutching _fieldsHoldingBarcode tagno
    foreach ($this->_biblioFields as $key => $value) {
		foreach ($this->_fieldsHoldingBarcode as $hkey){
			if ($key[0] != $hkey[0])
				continue;
			if (strncmp($key, $hkey, 3)!=0)
				continue;
			return true;
		}
	}
  ...
  function tags_extract_barcode(){ // jalg multi 5-7-2015 ALX
    foreach ($this->_biblioFields as $key => $value) {
		foreach ($this->_fieldsHoldingBarcode as $hkey){
			if ($key[0] != $hkey[0])
				continue;
			if (strncmp($key, $hkey, 4)!=0)
				continue;
			return $value;
		}
	}
